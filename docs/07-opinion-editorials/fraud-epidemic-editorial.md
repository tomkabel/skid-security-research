...
Miks on selline riskantne arhitektuur üldse lubatud? Siin tuleb vaadata SK ID Solutionsi omanikeringi: Swedbank, SEB ja Telia Eesti. Pangad ei ole siin ohvrid, vaid kasusaajad. Küberturvalisuse teadlane Arnis Paršovs on juhtinud tähelepanu sellele, et varasemad autentimisviisid (paroolikaardid, riistvaralised tokenid) olid kaugsotsiaalinseneeria (telefonitsi toimuvate petuskeemide) suhtes vastupidavamad, kuna need eeldasid koodide aktiivset ettelugemist, mitte passiivset kinnitusnuppu — see on kitsas väide konkreetse ründevektori kohta, mitte hinnang üldisele turvalisusele: paroolikaartidel endil olid tõsised nõrkused (haavatavus vahendusrünnetele, lihtne kopeerimine, puudulik mitteomistatavus) ja Smart-ID on krüptograafiliselt (PKI-põhiselt) tugevam lahendus. Pangad on aastate jooksul need vanemad viisid järk-järgult odava ja mugava äpi vastu vahetanud; kas see otsus tulenes eeskätt kuludest ja mugavusest või teadlikult võetud turvariskist, ei ole avalikult tõendatud — pankade endi selgitused (näiteks Smart-ID+ juurutuse tehnilise keerukuse kohta, ERR, jaanuar 2026) viitavad pigem esimesele, kuid käesolev artikkel väidab, et lõpptulemus — pettuseriski ülekandumine kliendile — on sama, olenemata motiivist.
...
Euroopa õigusraamistik nõuab turvalisust disainietapis, ehkki iga regulatsioon kohustab erinevat osapoolt: GDPR artikkel 25 nõuab vastutavatelt töötlejatelt "lõimitud ja vaikimisi andmekaitset" (*data protection by design and by default*), samal ajal kui eIDAS määruse artikkel 24 kohustab kvalifitseeritud usaldusteenuse pakkujaid, nagu SK ID Solutions, kasutama usaldusväärseid ja muutmiskindlaid süsteeme koos sobiva krüptograafiaga (artikkel 24 lõike 2 punkt e) ning rakendama riskipõhiseid organisatsioonilisi meetmeid (artikkel 24 lõike 2 punkt fa); need artiklid ei tee FIDO2/WebAuthn-tüüpi päritolu sidumist iseenesest kohustuslikuks ega tähenda, et RIA või TTJA oleks Smart-ID arhitektuuri madalamale turvatasemele ametlikult heaks kiitnud. Käesoleva artikli enda toimetuslik hinnang on, et Smart-ID arhitektuurijulgeolek on jäänud maha moodsast standardist (FIDO2/WebAuthn), mida kasutavad tehnoloogiahiiud nagu Google ja Apple — see on autori järeldus, mitte tsiteeritud regulatiivne otsus. Kui miljardite dollaritega tehnoloogiafirmad peavad päritolu sidumist (origin binding) elementaarseks, on põhjust küsida, miks Eesti regulaatorid pole seda seni Smart-ID puhul nõudnud.
