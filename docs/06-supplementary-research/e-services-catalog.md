...
**Government E-Services (Section 2):** Smart-ID+ was released for production use in June 2025; Estonia's state portals adopted it in February 2026 (roughly 8 months later), and the first commercial bank (Bigbank) followed in June 2026 (roughly 4 months after the state portals — see this repository's `research/smart-id-plus-eudiw-stall/report.md` for bank-by-bank rollout dates). The [RIA Memorandum](../04-regulatory-memoranda/ria-memorandum.md) supports a general market-failure argument — that RIA can mandate upgrades for its own portals but cannot compel banks — but does not itself establish the exact adoption interval cited here; that interval is this catalog's own calculation from the dates above, not a claim sourced from the memorandum.
...
**Critical Infrastructure (Section 8):** Estonia's X-Road data exchange layer connects government services, and the e-residency program extends the same PKI infrastructure to 170+ countries. This catalog treats "compromised Smart-ID authentication could become a pivot point into X-Road-connected infrastructure" as a structural hypothesis worth investigating, not a documented incident or established attack path — no source cited here demonstrates such a pivot has occurred.
