...
**Financial Services (Section 1):** Estonian banks are the primary Relying Parties for Smart-ID and the organizations with the most to lose from authentication failures. The €42M in aggregate fraud losses — reported alongside (not proven to be concentrated in) Smart-ID PIN-phishing scams — demonstrates the economic scale of the architectural vulnerabilities analyzed in [Smart-ID Security Analysis](../01-core-research/smartid-security-analysis.md). The ownership structure (Swedbank AS 25%, SEB Pank AS 25%, and Telia Eesti AS 50%, as shareholders in SK ID Solutions AS since its 2001 founding) is directly relevant to the conflict-of-interest analysis in the enforcement strategy documents.

**Government E-Services (Section 2):** Estonia's state portals mandated Smart-ID+ for state e-service login from February 26, 2026. LHV, the first commercial bank confirmed to have rolled out Smart-ID+ login across its Internet Bank, followed on 16 June 2026 — roughly four months later — but Swedbank and SEB's rollout dates were not confirmed at the time of writing, so "four months" describes the LHV data point specifically, not commercial banks as a class. This asymmetric adoption pattern (state leads, at least one major bank lags) is consistent with the market-failure argument in the [RIA Memorandum](../04-regulatory-memoranda/ria-memorandum.md), though the memorandum itself does not establish this specific interval. The state's ability to mandate security upgrades for its own portals — contrasted with its inability to compel banks — demonstrates the regulatory jurisdiction gap.
