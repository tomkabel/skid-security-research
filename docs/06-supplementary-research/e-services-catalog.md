...
**Critical Infrastructure (Section 8):** Estonia's X-Road (X-tee) data exchange layer connects government information systems using its own institutional AUTH/SIGN security-server certificates, issued to X-tee member organizations for system-to-system exchange. The e-Residency program uses separate personal authentication and signing certificates issued to individuals. Both sit within Estonia's broader PKI ecosystem administered by SK ID Solutions, but that shared administration does not establish a shared X-Road trust boundary or a documented authentication path from Smart-ID into X-Road. This catalog treats "compromised Smart-ID authentication could become a pivot point into X-Road-connected infrastructure" as a structural hypothesis worth investigating, conditional on evidence of documented integration and authorization between the two — not a documented incident or established attack path — no source cited here demonstrates such a pivot has occurred.
