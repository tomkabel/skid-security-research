...
**Financial Services (Section 1):** Estonian banks are the primary Relying Parties for Smart-ID and the organizations with the most to lose from authentication failures. Estonia's approximately €42 million in aggregate fraud losses for 2024 (Ministry of Justice and Digital Affairs crime statistics, covering phone scams, marketplace fraud, investment scams, and other fraud types generally, not a Smart-ID-specific breakdown) is cited here only as general context for the economic scale of fraud in Estonia, not as evidence apportioned to the architectural vulnerabilities analyzed in [Smart-ID Security Analysis](../01-core-research/smartid-security-analysis.md). The ownership structure (Swedbank AS, SEB Pank AS, and Telia Eesti AS as founding shareholders in SK ID Solutions) is directly relevant to the conflict-of-interest analysis in the enforcement strategy documents.
...
**Healthcare (Section 6):** The Allium UPI OÜ breach (700,000 records, half the population) and Apotheka breach are documented data breaches at healthcare-adjacent suppliers; this catalog lists Allium UPI OÜ's authentication method as "Unknown" (see table above), so these incidents should **not** be read as evidence of Smart-ID or authentication-layer compromise — the cited sources document a data breach at a supplier, not an authentication failure. The broader point that a breach at any single provider can expose health, genetic, or prescription data across a shared identity/data-exchange infrastructure is a structural hypothesis about blast radius, not a proven causal chain from these specific incidents.

**Critical Infrastructure (Section 8):** Estonia's X-Road (X-tee) data exchange layer connects government information systems using its own institutional AUTH/SIGN security-server certificates, issued to X-tee member organizations for system-to-system exchange. The e-Residency program uses separate personal authentication and signing certificates issued to individuals. Both sit within Estonia's broader PKI ecosystem — on X-Road specifically, trust services (e-seal and authentication certificates for security servers) are provided by both RIA (the Estonian Information System Authority) and SK ID Solutions, not by SK ID Solutions alone — but this does not establish a shared X-Road trust boundary or a documented authentication path from Smart-ID into X-Road. This catalog treats "compromised Smart-ID authentication could become a pivot point into X-Road-connected infrastructure" as a structural hypothesis worth investigating, conditional on evidence of documented integration and authorization between the two — not a documented incident or established attack path — no source cited here demonstrates such a pivot has occurred.

### How to Use This Catalog

- **For security researchers:** Prioritize targets by the "Data Sensitivity" and "Notable Incidents" columns. CRITICAL-rated services with documented breaches represent the highest analytical priority for this study. This catalog is a research-scoping reference, not a targeting directive: any testing of a named organization requires that organization's own prior written authorization, must never touch production data or live accounts, and must be reported through that organization's own security contact, bug-bounty program, or published vulnerability-disclosure policy — not this project's. This project's [responsible-disclosure timeline](../05-enforcement/responsible-disclosure-timeline.md) records this project's own reporting history with SK ID Solutions and Estonian regulators; it is background context, not an authorization for third-party testing and not a substitute for a named organization's own reporting process.
